...
the caller or the called function, obtains a new dimension. A similar issues
...
behavior in Check-and_abort mode are that the wrong set of contract annotations
...
However, inconsistent contract annotaitons have consequences on static analysis.
...
2. It is ok to have such incompatible declarations as long as the function is not
   odr-used. If it is, the behavior is undefined (UB).
